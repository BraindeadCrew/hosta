...
var saveFile = function saveFile(name, data, callback, count) {
  count = count || 0;
  if (count > 42) return callback(new Error('Unable to create a new random key name, is there any chances that we ran out of ramdomly generated hashes ?'));
  var downloadLink = path.resolve(config['files-path'], crypto.generateKey(8)+name);
  fs.writeFile(downloadLink, data, 'binary', function (err) {
    if (err) {
      console.error("Something went wrong while saving file " + downloadLink + ', gonna try again with another random key...');
